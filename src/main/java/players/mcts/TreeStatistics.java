...
public class TreeStatistics {
    ...
    public TreeStatistics(BasicTreeNode root) {
        Queue<BasicTreeNode> nodeQueue = new ArrayDeque<>();
        ...
        int oneAction = 0;
        while (!nodeQueue.isEmpty()) {
            BasicTreeNode node = nodeQueue.poll();
            if (node.depth < maxDepth) {
                nodesAtDepth[node.depth]++;
                if (node.terminalNode)
                    gameTerminalNodesAtDepth[node.depth]++;
                ...
                if (node.children.size() > maxActions)
                    maxActions = node.children.size();
                for (BasicTreeNode child : node.children.values().stream()
                        .filter(Objects::nonNull)
                        .filter(Objects::nonNull)
                        .collect(toList())) {
                    if (child != null)
